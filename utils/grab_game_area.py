...
def get_perspective_transform(stream, screen_resolution, ref_shape):
    """
    ...
    try:
        ...
        # 得到目标区域的四个角落
        rect, resized = get_region_corners(frame, ref_shape)
        ...
        # cv2.waitKey(0)
        return m, max_width, max_height, resized
    except Exception:
        print("except!")


def grab_game_area(stream: VideoStream, target_img):  # 传入一个VideoStream到这个function里
    ...
    # print(ref_img)
        ...
        print("cannot parse VideoStream frame")
    ...
    try:
        ...
        print("ref_shape does not exist")

    try:
        m, max_width, max_height, resized = get_perspective_transform(
            stream, screen_res, ref_shape)
        return resized

    except Exception:
        print("except in get_perspective_transform()")
